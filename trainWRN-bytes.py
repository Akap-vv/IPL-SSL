...
from models.agument import DataTransform,DataTransform_784
...
def main():
    ...
    parser.add_argument('--dataset', default='cifar10', type=str,
                        choices=['cifar10'],
                        help='dataset name')
    parser.add_argument('--num-labeled', type=int, default=25,
    ...
    parser.add_argument('--wdecay', default=5e-4, type=float,
                        help='weight decay')
    parser.add_argument('--isLBS', action='store_true', default=False,
                        help='data is LBS or Byte784')
    ...
    parser.add_argument('--seed', default=10, type=int,
                        help="random seed")
    parser.add_argument('--agu_level', default=2, type=int,
    ...
    # train_path = './data/CICIDS2017/train_norm.npy'
    # test_path = './data/CICIDS2017/test_norm.npy'
    # train_path = './data/ISCX2012/train_balan_1000.npy'
    # test_path = './data/ISCX2012/test_balan_1000.npy'
    # train_X, train_Y = data_reader(train_path)
    # test_X, test_Y = data_reader(test_path)
    train_X = np.load('./data/CICIoMT2024/train_28_28.npy')[:,:-1]
    # np.random.seed(202)
    # train_ind = np.arange(train_X.shape[0])
    # train_ind_choose =np.random.choice(train_ind,12000,replace=False)
    # train_X = train_X[train_ind_choose]
    train_Y = np.load('./data/CICIoMT2024/train_28_28.npy')[:,-1]
    test_X = np.load('./data/CICIoMT2024/test_28_28.npy')[:,:-1]
    test_Y = np.load('./data/CICIoMT2024/test_28_28.npy')[:,-1]
    # train_X = np.load('./data/Ton-IoT/train_784_4000.npy')[:, :-1]
    # train_Y = np.load('./data/Ton-IoT/train_784_4000.npy')[:, -1]
    # test_X = np.load('./data/Ton-IoT/test_784_1000.npy')[:, :-1]
    # test_Y = np.load('./data/Ton-IoT/test_784_1000.npy')[:, -1]
    ...
    # train_labeled_x = np.load('./data/CICIoMT2024/train_labeled_5per0.npy')[:, :-1]
    # train_labeled_y = np.load('./data/CICIoMT2024/train_labeled_5per0.npy')[:, -1]
...
def train_sup(args, labeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            logits_x, _, _ = model(inputs_x,isLBS=False)
            ...
            if not args.no_progress:
                ...
                p_bar.set_description(
                    "Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Loss: {loss:.4f}. ".format(
                        epoch=epoch + 1,
                        epochs=args.epochs,
                        batch=batch_idx + 1,
                        iter=args.eval_step,
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            inputs_u_w, inputs_u_s =DataTransform_784(inputs_u,args.agu_level)
            inputs_u_w1, inputs_u_s1 =DataTransform_784(inputs_u1,args.agu_level)
            ...
            logits, features, outs = model(inputs,isLBS=args.isLBS)
            ...
            logits1, features1, outs1 = model(input1,isLBS=args.isLBS)
            ...
            logits2, features2, outs2 = model(input2,isLBS=args.isLBS)
            del logits2
            ...
            if not args.no_progress:
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Time:{tt:.3f}. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. Loss_m: {loss_m:.4f}. ".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
        ...
        if args.use_ema:
            ...
            test_model = model
        if args.local_rank in [-1, 0]:
            test_loss, test_acc, test_time = test(args, test_loader, test_model, epoch)
            ...
            if is_best:
                ...
                if counter > args.early_stop:
                    print('EarlyStopping!!!   Saving Best_acc is {}'.format(best_acc))
                    print('avg_train_time:', Train_time[-1]/(epoch+1))
                    print('avg_test_time:', sum(Test_time)/len(Test_time))
                    break
                    # return best_acc
...
def train_agu(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            mask_probs.update(mask.mean().item())
            if not args.no_progress:
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Loss: {loss:.4f}. Loss_x: {loss_x:.4f}. Loss_u: {loss_u:.4f}. ".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
...
def test(args, test_loader, model, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            outputs, fea, outs = model(inputs,isLBS=args.isLBS)
            ...
            if not args.no_progress:
                test_loader.set_description("Test Iter: {batch:4}/{iter:4}. TTime: {tt:.3f}s. Loss: {loss:.4f}. top1: {top1:.2f}. ".format(
                    batch=batch_idx + 1,
    ...
    return losses.avg, top1.avg, test_time
